...
const DefaultCategoryDescription = dynamic(
  () => import("@/components/DefaultCategoryDescription"),
  { loading: () => <Spinner /> },
);
const DashBorder = dynamic(() => import("@/components/shared/DashBorder"));
const RatingListItem = dynamic(
  () => import("@/components/shared/table/RatingListItem"),
);
...
const ComparisonPage = ({
  ...
  categorySidebar,
  mostReadSidebar,
  featuredBrokersSidebar,
  forexSchoolSidebar,
  forexStrategySidebar,
  promotionSidebar,
  navigationSidebar,
  ...
  const doChangeSort = (field) => {
    const order =
      sorter.field === field && sorter.orderBy === "asc" ? "desc" : "asc";
    ...
    const allBrokerRes = axios
      .get(`${config.backendUrl}/broker`, { params })
      .then((res) => {
        ...
        <MDBox display="flex" flexDirection="column" gap={2}>
          {category && (
            <PageContent>
              ...
              <HtmlViewWrapper>
                <p>{i18n.entities.broker.text.broker_comparison_teaser}</p>
              </HtmlViewWrapper>
                    ...
                    <MDBox component="thead">
                      <TableRow>
                        ...
                        <DataTableHeadCell
                          px={1}
                          onClick={() =>
                            doChangeSort("broker_rating.overall_reviews")
                          }
                          sorted={sorter.orderBy === "asc" ? "desc" : "asc"}
                        >
                        ...
                        <DataTableHeadCell
                          px={1}
                          onClick={() =>
                            doChangeSort("broker_rating.overall_rating")
                          }
                          sorted={sorter.orderBy === "asc" ? "desc" : "asc"}
                        >
                            ...
                            <LazyLoad>
                              ...
                              >
                                <CardMedia
                                  component={"img"}
                                  image={
                                    row.broker_image_broker_logo[0]?.downloadUrl
                                  }
                                  alt={row.name}
                                  sx={{
                                    margin: 0,
...
export async function getStaticProps() {
  ...
  const [baseRes, categoryRes, pageRes] = await Promise.all([
    axios.get(`${config.backendUrl}/base`),
    axios.post(`${config.backendUrl}/category`, { url: "/broker-vergleich" }),
    axios.post(`${config.backendUrl}/general-page`, {
      url: "/broker-vergleich",
    }),
  ]);
  ...
  const allBrokerRes = await axios.get(`${config.backendUrl}/broker`, {
    params: {
      filter: filter,
      orderBy: "name_asc",
      limit: null,
